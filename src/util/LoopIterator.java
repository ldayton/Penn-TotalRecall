 ...
 *
 * <p>In the increasing order case, traversal starts at index x, ends at index x - 1, and the first
 * element of the array is visited after the last. In the decreasing order case, traversal starts at
 * index x, ends at index x + 1, and the last element of the array is visited after the first.
 *
 * <p>Does not support remove().
 *
...
public class LoopIterator<T> implements Iterator<T> {
     ...
     *
     * <p>The first element to be returned will be arr[index].
     *
     * @param arr The array to be iterated over
     * @param index The index at which to begin iteration
     * @param forward Whether iteration will take place in increasing order, decreasing order
     *     otherwise
    ...
    /**
     * Determines if there are more elements to return. If <tt>false</tt> is returned, calling
     * next() will lead to a NoSuchElementException.
     *
