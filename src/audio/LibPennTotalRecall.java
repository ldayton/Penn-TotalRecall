 ...
 * 
 * Replaces the C wrapper library with direct Java calls to FMOD Core API.
 * Maintains the same interface as the original LibPennTotalRecall for compatibility.
 * 
 * @author Yuvi Masory
 */
public final class LibPennTotalRecall {
	...
	interface FMODCore extends Library {
		...
		int FMOD_System_CreateSound(Pointer system, String name_or_data, int mode, Pointer exinfo, PointerByReference sound);
		...
		int FMOD_System_PlaySound(Pointer system, Pointer sound, Pointer channelgroup, int paused, PointerByReference channel);
	 ...
	 * 		-4 - inconsistent state (e.g. playbackInProgress())
	 * 
	...
	public int startPlayback(String canonicalPath, long startFrame, long endFrame) {
		synchronized (lock) {
			...
			try {
				...
				int result = fmod.FMOD_System_CreateSound(system, canonicalPath, FMOD_CREATESAMPLE, null, soundRef);
				if (result != FMOD_OK) {
				...
				result = fmod.FMOD_System_PlaySound(system, currentSound, null, 1, channelRef); // 1 = paused
				if (result != FMOD_OK) {
				...
				// Set start position
				result = fmod.FMOD_Channel_SetPosition(currentChannel, startFrame, FMOD_TIMEUNIT_PCM);
				if (result != FMOD_OK) {
					cleanup();
					return -1; // unspecified error
				}
