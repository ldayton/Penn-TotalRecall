...
/**
 * Stores a user-specific Preferences, as well as preference names, for persistent storage of
 * settings.
 *
...
public class UserPrefs {
     ...
     * the only Preferences object that should ever be used.
     */
    public static final Preferences prefs = Preferences.userNodeForPackage(UserPrefs.class);

    /**
     * The x-coordinate of MyFrame's top left corner. Changed only at program exit. Must map to a
     * non-negative integer.
     */
    public static final String windowXLocation = "WINDOW_X";

    /**
     * The y-coordinate of MyFrame's top left corner. Changed only at program exit. Must map to a
     * non-negative integer.
     */
    public static final String windowYLocation = "WINDOW_Y";

    /** The width of MyFrame. Changed only at program exit. Must map to a non-negative integer. */
    ...
    /** Default value for windowWidth Must be an integer. Must be non-negative. */
    public static final int defaultWindowWidth = 1000;

    /** The height of MyFrame. Changed only at program exit. Must map to a non-negative integer. */
    public static final String windowHeight = "WINDOW_HEIGHT";

    /** Default value for windowHeight Must be non-negative. */
    public static final int defaultWindowHeight = 500;

    /**
     * The location of the split pane divider in MySplitPane. Changed only at program exit. Must map
    ...
    /** Whether or not to warn user on exit. Must map to boolean true or false. */
    ...
    /** Whether or not to warn user when switching files. Must map to boolean true or false. */
    ...
    /** Lower bound for the bandpass filter. Must map to a non-negative integer. */
    public static final String minBandPass = "MIN_BAND_PASS";

    /** Default value for minBandPass. Must be non-negative. */
    public static final int defaultMinBandPass =
            1000; // phone company standard: 300, army standard: 400

    // (http://cnx.org/content/m15683/latest/), pyparse: 1000

    /** Upper bound for the bandpass filter. Must map to a non-negative integer. */
    public static final String maxBandPass = "MAX_BAND_PASS";

    /** Default value for maxBandPass. Must be non-negative and not less than defaultMinBandPass. */
    public static final int defaultMaxBandPass =
            16000; // phone company standard: 3600, army standard: 2800

    // (http://cnx.org/content/m15683/latest/), pyparse: 16000
