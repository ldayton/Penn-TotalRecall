...
public class Resampler {
     ...
     *
     * <p>This implements the "extract some of the samples for representation as pixels" logic from
     * WaveformBuffer.getValsToDraw() lines 390-400.
     *
    ...
    public double[] downsample(
            double[] samples, int skipInitialSamples, int targetLength, int numSamplesAvailable) {
        if (targetLength <= 0) {
        ...
        logger.debug(
                "Downsampled {} samples to {} pixels (increment={})",
                samples.length,
                targetLength,
                sampleIncrement);
        return valsToDraw;
     ...
     *
     * <p>This implements the "make the waveform prettier by smoothing the pixels" algorithm from
     * WaveformBuffer.getValsToDraw() lines 402-421. It removes isolated peaks and troughs that
