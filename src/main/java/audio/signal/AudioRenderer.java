 ...
 *
 * <p>This class handles the conversion of raw audio data into displayable
 * waveform representations, including filtering and envelope detection.
 */
public class AudioRenderer {
     ...
     */
    public double[] bandpassFilter(AudioDoubleDataSource audioSource, double minBand, 
                                 double maxBand, double[] outputSamples) {
        if (minBand < 0 || maxBand > 0.5 || minBand >= maxBand) {
            throw new IllegalArgumentException("Invalid bandpass range: " + minBand + " to " + maxBand);
        }
        
        BandPassFilter filter = new BandPassFilter(minBand, maxBand);
        filter.apply(audioSource).getData(outputSamples);
        
        logger.debug("Applied bandpass filter {}-{} Hz to {} samples", 
                     minBand, maxBand, outputSamples.length);
     ...
     *
     * <p>For each sample position, finds the maximum absolute amplitude within a symmetric
     * window around that position. This creates visually appealing waveforms by
     * emphasizing sustained loud sections while reducing isolated spikes.
     *
     * <p>This is the actual algorithm from WaveformBuffer.getValsToDraw() that makes
     * "waveform prettier by smoothing the audio data (~60ms)".
     *
    ...
    public double[] envelopeSmooth(double[] samples, int windowSize) {
        ...
        logger.debug("Applied envelope smoothing (window={}) to {} samples", 
     ...
     *
     * <p>Looks for maximum amplitude that occurs in adjacent samples,
     * representing sustained audio content versus brief spikes.
     *
     * <p>This is the "biggestConsecutivePixelVals" algorithm from WaveformBuffer
     * that finds "largest value that 2 consecutive pixels will actually draw at".
     *
     * @param samples sample data to analyze (typically pixel-resolution data)
     * @param skipInitialSamples number of initial samples to skip (WaveformBuffer uses pixelsPerSecond/2)
