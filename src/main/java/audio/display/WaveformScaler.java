 ...
 *
 * <p>This class handles the mathematical operations needed to scale audio amplitude
 * values for optimal visual display, ensuring good use of available screen space.
 */
public class WaveformScaler {
     ...
     *
     * <p>This is the exact Y-scaling algorithm from WaveformBuffer lines 276-281.
     * It determines yScale by finding the largest sustained amplitude and scaling
     * to use most of the available display height.
     *
     ...
     */
    public double getPixelScale(double[] samples, int displayHeight, double biggestConsecutivePixelVals) {
        if (displayHeight <= 0) {
        ...
        logger.debug("Calculated pixel scale: {} (peak={}, height={})", 
                     yScale, biggestConsecutivePixelVals, displayHeight);
        return yScale;
    }
    ...
    public double[] normalize(double[] samples, double targetPeak) {
        ...
        logger.debug("Normalized {} samples: peak {} -> {} (scale={})", 
                     samples.length, currentPeak, targetPeak, scaleFactor);
        return samples;
    }
    ...
    public static class AmplitudeStats {
        ...
        public AmplitudeStats(double min, double max, double mean, 
                            double rms, double peak, int sampleCount) {
            this.min = min;
            this.max = max;
            this.mean = mean;
            this.rms = rms;
        ...
        public String toString() {
            return String.format("AmplitudeStats{min=%.3f, max=%.3f, mean=%.3f, rms=%.3f, peak=%.3f, n=%d}",
