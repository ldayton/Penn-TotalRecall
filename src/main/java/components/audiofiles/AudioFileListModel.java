 ...
 * repetitions.
 *
 * <p>The sorting strategy anticipates that any time many files are added consecutively, they will
 * be added as a batch. This corresponds to the user opening a directory with <code>
 * OpenAudioLocationAction</code> or dragging many files (or a directory) onto the application. To
 * encourage fast sorting, no method is provided for adding one element individually.
 */
...
public class AudioFileListModel implements ListModel<AudioFile>, ChangeListener {
     ...
     *
     * <p>Since removing elements cannot make a sorted list unsorted, no sorting is performed after
     * the removal. However, all registered listeners are notified of a <code>
     * ListDataEvent.INTERVAL_REMOVED</code> event.
     *
     ...
     *
     * <p>Sets this <code>AudioFileListModel</code> to listen to completion status changes in any
     * files added. After all files are added, the list is re-sorted using {@link
