 ...
 * 
 * This test creates actual Swing components and verifies that:
 * 1. FlatLaf is properly set
 * 2. No Aqua-related exceptions occur during component creation and window activation
 * 3. The Look and Feel remains FlatLaf throughout the test
 */
    ...
    void fullApplicationStartupShouldUseFlatLafWithoutAquaErrors() throws Exception {
        ...
        CompletableFuture<Void> startupFuture = CompletableFuture.runAsync(() -> {
            try {
                ...
                logger.info("Look and Feel before startApplication: {}", currentLaf);
                assertTrue(currentLaf.contains("flatlaf"), 
                    "Should be using FlatLaf before startApplication, but got: " + currentLaf);
                ...
                assertTrue(currentLaf.contains("flatlaf"), 
                    "Should be using FlatLaf after startApplication, but got: " + currentLaf);
        ...
        SwingUtilities.invokeAndWait(() -> {
            ...
            for (Window window : windows) {
                logger.info("Window: {} - visible: {}, displayable: {}", 
                    window.getClass().getSimpleName(), window.isVisible(), window.isDisplayable());
            }
            ...
            for (Window window : windows) {
                if (window.isDisplayable() && window.isVisible()) {
                    ...
                    logger.info("✅ Found visible main window: " + window.getClass().getSimpleName());
                    ...
                    assertTrue(windowLaf.contains("flatlaf"), 
        ...
        SwingUtilities.invokeAndWait(() -> {
            ...
            for (Window window : windows) {
                if (window.isVisible()) {
                    ...
                    window.requestFocus();
                    window.toFront();
                    logger.info("Triggered window activation for: " + window.getClass().getSimpleName());
                }
            }
        });
    ...
    void swingComponentCreationShouldNotTriggerAquaErrors() throws Exception {
        ...
        SwingUtilities.invokeAndWait(() -> {
            try {
                ...
                assertTrue(lafAfterCreation.contains("flatlaf"), 
